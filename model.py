...
class DifficultIntubationModel:
    ...
    def generate_initial_medical_data(self):
        ...
        data = {
            ...
            'neck_circumference': np.random.normal(38, 4, n_samples),
            'thyromental_distance': np.random.normal(6.5, 1.5, n_samples),
            'interincisor_distance': np.random.normal(4.5, 1.0, n_samples),
        ...
        difficulty_prob = (
            (df['mallampati'] - 1) * 0.15 +
            (df['neck_circumference'] > 40) * 0.2 +
            (df['thyromental_distance'] < 6) * 0.15 +
            (df['stop_bang_score'] >= 3) * 0.2 +
            (df['upper_lip_bite_test'] == 3) * 0.15 +
            (df['neck_mobility'] == 0) * 0.1 +
            (df['interincisor_distance'] < 3) * 0.15
        )
        ...
        for _, row in df.iterrows():
            data_point = IntubationData(
                ...
                cormack_grade=np.random.choice([3, 4]) if row['difficult_intubation'] else np.random.choice([1, 2]),
